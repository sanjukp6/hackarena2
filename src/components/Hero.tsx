    ...
    <div className="flex gap-3 md:gap-4">
      {Object.entries(timeLeft).map(([key, value]) => (
        <motion.div
          ...
          className="relative"
        >
          <div className="w-16 h-16 md:w-20 md:h-20 bg-black/50 backdrop-blur-sm border border-orange-500/30 rounded-lg flex flex-col items-center justify-center relative overflow-hidden group hover:border-orange-400/60 transition-colors">
            <div className="absolute inset-0 bg-gradient-to-b from-orange-500/10 to-transparent" />
            <span className="font-display text-2xl md:text-3xl font-bold text-orange-400 relative z-10">
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {[
            { icon: Calendar, text: '28th Feb - 1st March, 2026', color: 'orange' },
            { icon: MapPin, text: 'BIET Davangere', color: 'red' },
            { icon: Clock, text: '24 Hours', color: 'amber' },
          ].map(({ icon: Icon, text, color }) => (
            <motion.div
              key={text}
              whileHover={{ scale: 1.05, y: -5 }}
              className={`flex items-center gap-3 px-5 py-3 bg-black/50 backdrop-blur-sm rounded-xl border transition-all duration-300 ${color === 'orange'
                ? 'border-orange-500/30 hover:border-orange-400 hover:shadow-lg hover:shadow-orange-500/20'
                : color === 'red'
                  ? 'border-red-500/30 hover:border-red-400 hover:shadow-lg hover:shadow-red-500/20'
                  : 'border-amber-500/30 hover:border-amber-400 hover:shadow-lg hover:shadow-amber-500/20'
                }`}
            >
              <Icon className={`w-5 h-5 ${color === 'orange' ? 'text-orange-400' : color === 'red' ? 'text-red-400' : 'text-amber-400'
                }`} />
